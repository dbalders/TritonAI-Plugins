...
export class MicrosoftGraphProvider {
    ...
    async #request(input, init, maximumBytes, inspectResponse) {
        if (this.#closed)
            throw new Error("Microsoft 365 Read is closed.");
    ...
    async #graph(path, accessToken, signal) {
        const { response, json } = await this.#request(`${GRAPH_API_ROOT}${path}`, { headers: { authorization: `Bearer ${accessToken}` }, signal: signal ?? null }, GRAPH_RESPONSE_BYTES, (received) => {
    ...
    async invoke(toolName, input, context) {
        ...
        const access = this.#accessToken;
        if (!access || access.expiresAt - ACCESS_TOKEN_SKEW_MS <= Date.now()) {
            throw new IntegrationProviderPublicError("The Microsoft 365 Read session needs a safe refresh. Disconnect and reconnect.");
        }
        const generation = this.#generation;
        if (toolName === "microsoft365.mail.search") {
            ...
            const result = await this.#graph(`/me/messages?${params.toString()}`, access.value, context?.signal);
            if (this.#closed ||
                this.#disconnecting ||
                this.#uncertainCredentialState ||
                generation !== this.#generation) {
        ...
        if (toolName === "microsoft365.calendar.events") {
            if (!access.grantedScopes.includes(CAPABILITY_SCOPES["calendar.read"])) {
                throw new IntegrationProviderPublicError("Microsoft 365 Read calendar access is not granted.");
            }
            ...
            const end = isoTimestamp(values.end ?? new Date(Date.parse(start) + 7 * 86_400_000).toISOString());
            ...
            const result = await this.#graph(`/me/calendarView?${params.toString()}`, access.value, context?.signal);
            if (this.#closed ||
                this.#disconnecting ||
                this.#uncertainCredentialState ||
                generation !== this.#generation) {
