...
const OAUTH_SCOPE_SET = new Set(OAUTH_SCOPES);
const CAPABILITIES = [
    "credential.read",
    "data-table.read",
    "data-table.write",
...
const BoundedId = Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(1_024), Schema.isPattern(/^[^\p{Cc}\s]+$/u));
...
const OptionalQuery = Schema.optionalKey(Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(512)));
const OptionalProjectId = Schema.optionalKey(BoundedId);
const OptionalLimit100 = Schema.optionalKey(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 100 })));
const OptionalLimit200 = Schema.optionalKey(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 200 })));
const JsonObject = Schema.Record(Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(256)), Schema.Unknown);
const EmptyInput = Schema.Record(Schema.String, Schema.Never);
const SearchWorkflowsInput = Schema.Struct({
    query: OptionalQuery,
    projectId: OptionalProjectId,
    tags: Schema.optionalKey(Schema.Array(BoundedText).check(Schema.isMinLength(1), Schema.isMaxLength(50))),
    limit: OptionalLimit200,
    sortBy: Schema.optionalKey(Schema.Literals([
        "updatedAt:desc",
        "updatedAt:asc",
        "createdAt:desc",
        "createdAt:asc",
...
const ExecuteWorkflowInput = Schema.Struct({
    ...
    inputs: Schema.optionalKey(Schema.Union([
        ...
        Schema.Struct({
            ...
            webhookData: Schema.Struct({
                method: Schema.optionalKey(Schema.Literals(["GET", "POST", "PUT", "DELETE", "PATCH", "HEAD", "OPTIONS"])),
...
const GetExecutionInput = Schema.Struct({
    ...
    nodeNames: Schema.optionalKey(Schema.Array(BoundedText).check(Schema.isMinLength(1), Schema.isMaxLength(100))),
    truncateData: Schema.optionalKey(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 10_000 }))),
});
const SearchExecutionsInput = Schema.Struct({
    workflowId: Schema.optionalKey(BoundedId),
    status: Schema.optionalKey(Schema.Array(Schema.Literals([
        "canceled",
        "crashed",
        "error",
        "new",
    ...
    startedAfter: Schema.optionalKey(Schema.String.check(Schema.isMinLength(20), Schema.isMaxLength(64))),
    startedBefore: Schema.optionalKey(Schema.String.check(Schema.isMinLength(20), Schema.isMaxLength(64))),
    limit: OptionalLimit200,
...
const GetSdkReferenceInput = Schema.Struct({
    section: Schema.optionalKey(Schema.Literals([
        "patterns",
        "patterns_detailed",
        "expressions",
        "functions",
...
const NodeConfiguration = Schema.Struct({
    ...
    type: BoundedText,
    typeVersion: Schema.optionalKey(Schema.Finite.check(Schema.isBetween({ minimum: 0, maximum: 10_000 }))),
...
const CreateWorkflowInput = Schema.Struct({
    ...
    versionName: Schema.optionalKey(Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(80))),
    versionDescription: Schema.optionalKey(Schema.String.check(Schema.isMaxLength(1_000))),
    projectId: OptionalProjectId,
    folderId: Schema.optionalKey(BoundedId),
});
const Position = Schema.Array(Schema.Finite.check(Schema.isBetween({ minimum: -1_000_000, maximum: 1_000_000 }))).check(Schema.isMinLength(2), Schema.isMaxLength(2));
const NodeCredentials = Schema.Record(BoundedText, Schema.Struct({ id: Schema.optionalKey(BoundedId), name: BoundedText }));
...
const ConnectionFields = {
    source: BoundedText,
    target: BoundedText,
    sourceIndex: Schema.optionalKey(Schema.Int.check(Schema.isBetween({ minimum: 0, maximum: 1_000 }))),
    targetIndex: Schema.optionalKey(Schema.Int.check(Schema.isBetween({ minimum: 0, maximum: 1_000 }))),
    connectionType: Schema.optionalKey(BoundedText),
};
const UpdateSettings = Schema.Struct({
    onError: Schema.optionalKey(Schema.Literals(["stopWorkflow", "continueRegularOutput", "continueErrorOutput"])),
    ...
    waitBetweenTries: Schema.optionalKey(Schema.Int.check(Schema.isBetween({ minimum: 0, maximum: 5_000 }))),
    ...
    saveExecutionProgress: Schema.optionalKey(Schema.Union([Schema.Boolean, Schema.Literal("DEFAULT")])),
    saveManualExecutions: Schema.optionalKey(Schema.Union([Schema.Boolean, Schema.Literal("DEFAULT")])),
    ...
    executionTimeout: Schema.optionalKey(Schema.Int.check(Schema.isBetween({ minimum: -1, maximum: 31_536_000 }))),
    timeSavedPerExecution: Schema.optionalKey(Schema.Int.check(Schema.isBetween({ minimum: 0, maximum: 1_000_000 }))),
    callerPolicy: Schema.optionalKey(Schema.Literals(["any", "none", "workflowsFromAList", "workflowsFromSameOwner"])),
...
const UpdateOperation = Schema.Struct({
    ...
    path: Schema.optionalKey(Schema.String.check(Schema.isMinLength(2), Schema.isMaxLength(1_024), Schema.isPattern(/^\//u))),
    ...
    names: Schema.optionalKey(Schema.Array(BoundedText).check(Schema.isMinLength(1), Schema.isMaxLength(100))),
    nodeGroups: Schema.optionalKey(Schema.Array(Schema.Struct({
        id: Schema.optionalKey(BoundedId),
        name: BoundedText,
...
const ColumnName = Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(63), Schema.isPattern(/^[A-Za-z][A-Za-z0-9_]*$/u));
const CreateDataTableInput = Schema.Struct({
    projectId: BoundedId,
    name: Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(128)),
    columns: Schema.Array(Schema.Struct({ name: ColumnName, type: ColumnType })).check(Schema.isMinLength(1), Schema.isMaxLength(250)),
});
const DataTableIdentity = { dataTableId: BoundedId, projectId: BoundedId };
...
const AddDataTableRowsInput = Schema.Struct({
    ...DataTableIdentity,
    rows: Schema.Array(Schema.Record(ColumnName, DataTableScalar)).check(Schema.isMinLength(1), Schema.isMaxLength(1_000)),
});
...
const REVIEWED_TOOLS = [
    reviewedTool("search_workflows", "Search workflow previews visible to the connected n8n user.", SearchWorkflowsInput, "workflow.read"),
    reviewedTool("get_workflow_details", "Read one accessible workflow with sanitized nodes and trigger guidance.", WorkflowIdInput, "workflow.read"),
    reviewedTool("get_workflow_history", "Read bounded version history for one accessible workflow.", WorkflowHistoryInput, "workflow.read"),
    reviewedTool("get_workflow_version", "Read one exact historical workflow version.", WorkflowVersionInput, "workflow.read"),
    reviewedTool("execute_workflow", "Execute an accessible workflow in manual or production mode.", ExecuteWorkflowInput, "workflow.execute", { readOnly: false, destructive: true, idempotent: false, openWorld: true }),
    reviewedTool("test_workflow", "Test workflow logic with bounded pin data.", TestWorkflowInput, "workflow.execute", { readOnly: false, destructive: true, idempotent: false }),
    reviewedTool("prepare_workflow_pin_data", "Prepare pin-data schemas for testing one workflow.", WorkflowIdInput, "workflow.execute"),
    reviewedTool("publish_workflow", "Publish an accessible workflow version.", PublishWorkflowInput, "workflow.write", { readOnly: false, idempotent: true }),
    reviewedTool("unpublish_workflow", "Unpublish an accessible workflow.", WorkflowIdInput, "workflow.write", { readOnly: false, idempotent: true }),
    reviewedTool("search_projects", "Search projects visible to the connected n8n user.", SearchProjectsInput, "project.read"),
    reviewedTool("search_folders", "Search folders within one accessible project.", SearchFoldersInput, "project.read"),
    reviewedTool("list_workflow_tags", "List available workflow tags.", ListTagsInput, "tag.read"),
    reviewedTool("get_workflow_execution", "Read one accessible execution with optionally bounded result data.", GetExecutionInput, "execution.read"),
    reviewedTool("search_workflow_executions", "Search accessible workflow executions.", SearchExecutionsInput, "execution.read"),
    reviewedTool("list_credentials", "List accessible credential names and metadata without secret values.", ListCredentialsInput, "credential.read"),
    reviewedTool("list_n8n_connect_services", "List n8n Connect managed-credential coverage without credential values.", EmptyInput, "credential.read"),
    reviewedTool("get_workflow_sdk_reference", "Read reviewed n8n Workflow SDK reference material.", GetSdkReferenceInput, "workflow.read"),
    reviewedTool("search_nodes", "Search n8n node types by bounded queries.", SearchNodesInput, "workflow.read"),
    reviewedTool("get_node_types", "Read exact TypeScript definitions for selected n8n node types.", GetNodeTypesInput, "workflow.read"),
    reviewedTool("get_workflow_best_practices", "Read n8n best practices for a workflow technique.", BestPracticesInput, "workflow.read"),
    reviewedTool("explore_node_resources", "Resolve resources for one node using an accessible credential.", ExploreNodeResourcesInput, "credential.read", { openWorld: true }),
    reviewedTool("validate_workflow", "Validate bounded Workflow SDK code without saving it.", ValidateWorkflowInput, "workflow.read"),
    reviewedTool("validate_node_config", "Validate bounded node configurations without saving them.", ValidateNodeConfigInput, "workflow.read"),
    reviewedTool("create_workflow_from_code", "Create a workflow from validated Workflow SDK code.", CreateWorkflowInput, "workflow.write", { readOnly: false, idempotent: false }),
    reviewedTool("update_workflow", "Atomically apply a bounded ordered operation batch to a workflow.", UpdateWorkflowInput, "workflow.write", { readOnly: false, destructive: true, idempotent: false }),
    reviewedTool("archive_workflow", "Archive an accessible workflow.", WorkflowIdInput, "workflow.write", { readOnly: false, destructive: true, idempotent: true }),
    reviewedTool("restore_workflow_version", "Restore an accessible workflow from one exact historical version.", WorkflowVersionInput, "workflow.write", { readOnly: false, destructive: true, idempotent: false }),
    reviewedTool("search_data_tables", "Search data tables visible to the connected n8n user.", SearchDataTablesInput, "data-table.read"),
    reviewedTool("create_data_table", "Create a data table in an accessible project.", CreateDataTableInput, "data-table.write", { readOnly: false, idempotent: false }),
    reviewedTool("add_data_table_column", "Add a column to an accessible data table.", AddDataTableColumnInput, "data-table.write", { readOnly: false, idempotent: false }),
    reviewedTool("rename_data_table_column", "Rename a column in an accessible data table.", RenameDataTableColumnInput, "data-table.write", { readOnly: false, idempotent: true }),
    reviewedTool("delete_data_table_column", "Permanently delete a data-table column and its data.", DeleteDataTableColumnInput, "data-table.write", { readOnly: false, destructive: true, idempotent: false }),
    reviewedTool("rename_data_table", "Rename an accessible data table.", RenameDataTableInput, "data-table.write", { readOnly: false, idempotent: true }),
    reviewedTool("add_data_table_rows", "Insert bounded rows into an accessible data table.", AddDataTableRowsInput, "data-table.write", { readOnly: false, idempotent: false }),
];
export const N8N_TOOLS = REVIEWED_TOOLS;
...
function validateServerUrl(value) {
    ...
    if (url.toString() !== REVIEWED_SERVER_URL ||
        url.protocol !== "https:" ||
        url.username !== "" ||
        url.password !== "" ||
        url.search !== "" ||
...
function capabilitiesFromScopes(scopes) {
    ...
    if (scopes.includes("credential:read"))
        capabilities.add("credential.read");
    if (scopes.includes("project:read"))
        capabilities.add("project.read");
    if (scopes.includes("tag:read"))
        capabilities.add("tag.read");
    if (scopes.includes("workflow:read"))
        capabilities.add("workflow.read");
    if (scopes.includes("workflow:write"))
        capabilities.add("workflow.write");
    if (scopes.includes("workflow:execute"))
        capabilities.add("workflow.execute");
    if (scopes.includes("execution:read"))
        capabilities.add("execution.read");
    if (scopes.includes("dataTable:read"))
        capabilities.add("data-table.read");
    if (scopes.includes("dataTable:write"))
        capabilities.add("data-table.write");
    return [...capabilities].toSorted();
}
function parseCredential(bytes, serverUrl) {
    let parsed;
    try {
...
function parseJson(bytes, label = "n8n response") {
    try {
...
function parseMcpPayload(response, bytes) {
    ...
    if (contentType.includes("application/json"))
        return parseJson(bytes, "n8n MCP response");
    ...
    if (events.length !== 1)
        throw new Error("n8n MCP returned an ambiguous event stream.");
    ...
    for (const line of lines) {
        ...
        if (line.trim() === "" || line.startsWith("id:") || line.startsWith("retry:"))
            continue;
        throw new Error("n8n MCP returned an invalid event stream.");
    }
    if (dataLines.length === 0)
        throw new Error("n8n MCP event stream omitted response data.");
    ...
    for (const key of Object.keys(record)
        ...
        if (key === "properties") {
            ...
            if (!properties || typeof properties !== "object" || Array.isArray(properties))
                continue;
            normalized.properties = Object.fromEntries(Object.keys(properties)
                .toSorted()
                .map((name) => [name, schemaContract(properties[name])]));
            continue;
        }
...
function validateToolInventory(value) {
    ...
    for (const raw of result.tools) {
        ...
        if (actual.has(name))
            throw new Error("n8n MCP returned duplicate tools.");
    ...
    if (actual.size === 0 || [...actual.keys()].some((name) => !reviewedNames.has(name))) {
        throw new IntegrationProviderPublicError("n8n MCP tools changed from the reviewed catalog. Update the TritonAI n8n plugin before use.");
    }
    for (const reviewed of REVIEWED_TOOLS) {
        ...
        if (stableJson(upstream.inputSchema) !== stableJson(expectedSchema(reviewed))) {
            throw new IntegrationProviderPublicError(`n8n MCP schema changed for ${reviewed.upstreamName}. Update the TritonAI n8n plugin before use.`);
        }
        const annotations = upstream.annotations;
        if (!annotations || typeof annotations !== "object" || Array.isArray(annotations)) {
            throw new IntegrationProviderPublicError(`n8n MCP effect metadata changed for ${reviewed.upstreamName}.`);
        }
        const hints = annotations;
        if (hints.readOnlyHint !== reviewed.readOnly ||
...
export class N8nProvider {
    ...
    async #discover(signal) {
        const protectedUrl = new URL(`/.well-known/oauth-protected-resource${this.#server.pathname}`, this.#server.origin);
        const { response: protectedResponse, json: resource } = await this.#requestJson(protectedUrl.toString(), { method: "GET", headers: { accept: "application/json" }, signal: signal ?? null }, METADATA_RESPONSE_BYTES);
        if (!protectedResponse.ok)
            throw new Error("n8n OAuth protected-resource discovery failed.");
        if (resource.resource !== this.#server.toString()) {
            throw new Error("n8n OAuth resource metadata did not bind the configured server.");
        }
        if (!Array.isArray(resource.bearer_methods_supported) ||
        ...
        if (!Array.isArray(resource.authorization_servers) ||
            resource.authorization_servers.length !== 1) {
            throw new Error("n8n OAuth authorization server metadata is invalid.");
        }
        const issuer = sameOriginEndpoint(resource.authorization_servers[0], "/", this.#server.origin, "n8n OAuth authorization server").replace(/\/$/u, "");
        const metadataUrl = new URL("/.well-known/oauth-authorization-server", issuer);
        const { response, json } = await this.#requestJson(metadataUrl.toString(), { method: "GET", headers: { accept: "application/json" }, signal: signal ?? null }, METADATA_RESPONSE_BYTES);
        if (!response.ok || json.issuer !== issuer) {
            throw new Error("n8n OAuth authorization metadata is invalid.");
        }
        exactStringSet(json.scopes_supported, OAUTH_SCOPE_SET, "n8n OAuth authorization scope metadata");
        ...
        return {
            issuer,
            authorizationEndpoint: sameOriginEndpoint(json.authorization_endpoint, "/mcp-oauth/authorize", this.#server.origin, "n8n OAuth authorization endpoint"),
            tokenEndpoint: sameOriginEndpoint(json.token_endpoint, "/mcp-oauth/token", this.#server.origin, "n8n OAuth token endpoint"),
            registrationEndpoint: sameOriginEndpoint(json.registration_endpoint, "/mcp-oauth/register", this.#server.origin, "n8n OAuth registration endpoint"),
            revocationEndpoint: sameOriginEndpoint(json.revocation_endpoint, "/mcp-oauth/revoke", this.#server.origin, "n8n OAuth revocation endpoint"),
        };
    }
    async #registerClient(discovery, redirectUri, signal) {
        ...
        const { response, json } = await this.#requestJson(discovery.registrationEndpoint, {
            method: "POST",
    ...
    #writeCredential(credential, signal) {
        return Effect.runPromise(this.#secrets.set(N8N_SECRET_SUFFIX, encoder.encode(JSON.stringify(credential))), { signal });
    }
    ...
    async #mcpRpc(access, method, params, signal, maximumBytes, timeoutMs = this.#requestTimeoutMs, notification = false) {
        ...
        const { response, bytes } = await this.#request(this.#server.toString(), { method: "POST", headers, body, signal }, maximumBytes, timeoutMs);
        if (response.status === 401) {
            this.#accessToken = null;
            this.#sessionId = null;
            this.#sessionVerified = false;
            this.#availableTools = new Set();
            throw new IntegrationProviderPublicError("n8n authorization expired. Reconnect if refresh fails.");
        }
        ...
        if (!response.ok) {
            if (response.status === 429) {
                throw new IntegrationProviderPublicError("n8n is rate limiting MCP requests. Try again later.");
            }
            if (response.status === 403) {
                throw new IntegrationProviderPublicError("n8n denied this operation for the connected user.");
            }
            throw new IntegrationProviderPublicError("n8n MCP could not complete the request.");
        }
        const returnedSession = response.headers.get("mcp-session-id");
        if (returnedSession !== null) {
            if (returnedSession.length === 0 ||
        ...
        if (raw.error !== undefined) {
            ...
            if (!Number.isInteger(error.code))
                throw new Error("n8n MCP returned an invalid error.");
            throw new IntegrationProviderPublicError("n8n MCP rejected the request.");
        }
        if (!("result" in raw))
            throw new Error("n8n MCP response omitted its result.");
        return raw.result;
    }
    async #initializeSession(access, signal) {
        await this.#serializeSession(async () => {
            if (this.#sessionVerified)
                return;
            this.#sessionId = null;
            const initialize = asRecord(await this.#mcpRpc(access, "initialize", {
                protocolVersion: MCP_PROTOCOL_VERSION,
                capabilities: {},
                clientInfo: { name: "TritonAI Harness", version: "1.0.0" },
            }, signal, MCP_CONTROL_RESPONSE_BYTES), "n8n MCP initialize result");
            if (initialize.protocolVersion !== MCP_PROTOCOL_VERSION ||
                ...
                throw new IntegrationProviderPublicError("n8n MCP protocol changed from the reviewed version.");
            }
            await this.#mcpRpc(access, "notifications/initialized", undefined, signal, METADATA_RESPONSE_BYTES, this.#requestTimeoutMs, true);
            const collected = [];
            let cursor;
            for (let page = 0; page < MAX_MCP_PAGES; page += 1) {
                const result = asRecord(await this.#mcpRpc(access, "tools/list", cursor === undefined ? undefined : { cursor }, signal, MCP_CONTROL_RESPONSE_BYTES), "n8n MCP tools/list result");
            ...
            if (cursor !== undefined)
                throw new Error("n8n MCP tool inventory pagination is too large.");
    ...
    #parseTokenResponse(json, clientId, discovery, existingRefreshToken) {
        ...
        if (!refreshToken)
            throw new Error("n8n OAuth did not issue renewable access.");
    ...
    async connect(capabilities, context, submission) {
        ...
        if (this.#uncertainCredentialState)
            throw new Error("n8n credential state is uncertain.");
        ...
        const flow = await this.#startFlowListener({ flowId, state, codeVerifier, discovery, expiresAt, generation }, context?.signal);
        try {
        ...
        authorizationUrl.searchParams.set("code_challenge", NodeCrypto.createHash("sha256").update(codeVerifier, "ascii").digest("base64url"));
    ...
    prepare(context) {
        return this.#serializeCredential(async () => {
            ...
            try {
                ...
                responseSettled = true;
                if (!response.ok) {
                    throw new IntegrationProviderPublicError("n8n access could not be refreshed. Disconnect and reconnect.");
                }
                credentialIssued = true;
                const parsed = this.#parseTokenResponse(json, credential.clientId, discovery, credential.refreshToken);
    ...
    async invoke(toolName, input, context) {
        ...
        if (!reviewed)
            throw new IntegrationProviderPublicError("Unknown n8n tool.");
        ...
        if (!access ||
            ...
            this.#disconnecting ||
            this.#uncertainCredentialState) {
            throw new IntegrationProviderPublicError("n8n access is not prepared. Reconnect if this continues.");
        }
        if (!this.#availableTools.has(reviewed.upstreamName)) {
            throw new IntegrationProviderPublicError("This n8n tool is not available under the connected user's grant or instance configuration.");
        }
        ...
        if (!signal)
            throw new Error("n8n invocation requires a cancellation signal.");
        const timeout = reviewed.upstreamName === "test_workflow" ? TEST_REQUEST_TIMEOUT_MS : this.#requestTimeoutMs;
        const call = async () => {
            const result = asRecord(await this.#mcpRpc(access, "tools/call", { name: reviewed.upstreamName, arguments: decoded }, signal, MCP_TOOL_RESPONSE_BYTES, timeout), "n8n MCP tool result");
