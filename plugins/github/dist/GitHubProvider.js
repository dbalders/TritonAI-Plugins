...
const Repo = Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(100), Schema.isPattern(/^[A-Za-z0-9_.-]+$/u)).annotate({ description: "Exact GitHub repository name." });
...
const ActionsRunsListInput = Schema.Struct({
    ...
    status: Schema.optionalKey(Schema.Literals([
        "completed",
        "action_required",
        "cancelled",
        "failure",
...
export const GITHUB_TOOLS = [
    ...
    tool("github.installations.list", "List bounded GitHub App installations visible to this user token.", InstallationsListInput),
    tool("github.repositories.list", "List bounded repositories granted to one exact GitHub App installation.", RepositoriesListInput),
    tool("github.repositories.get", "Read one exact repository.", RepositoryGetInput),
    tool("github.repositories.search", "Search repositories with a bounded GitHub search query.", RepositorySearchInput),
    tool("github.contents.get", "Read one exact repository file no larger than one megabyte.", ContentsGetInput),
    tool("github.code.search", "Search code in one exact repository.", CodeSearchInput),
    tool("github.issues.list", "List bounded issues and pull-request issue records in one repository.", IssuesListInput),
    tool("github.issues.search", "Search issues and pull requests in one exact repository.", IssuesSearchInput),
    tool("github.issues.get", "Read one exact issue or pull-request issue record.", NumberInput),
    tool("github.issues.comments.list", "List bounded conversation comments for one issue or pull request.", Schema.Struct({ ...ownerRepo, number: IssueNumber, ...pagination })),
    tool("github.issues.create", "Create one bounded issue.", IssueCreateInput, false),
    tool("github.issues.update", "Narrowly update one issue without deleting it.", IssueUpdateInput, false, true),
    tool("github.issues.comment.create", "Add one bounded comment to an issue.", CommentCreateInput, false),
    ...
    tool("github.pulls.reviews.list", "List bounded review metadata for one pull request.", Schema.Struct({ ...ownerRepo, number: IssueNumber, ...pagination })),
    tool("github.pulls.review-comments.list", "List bounded inline review comments for one pull request.", Schema.Struct({ ...ownerRepo, number: IssueNumber, ...pagination })),
    tool("github.pulls.create", "Create a pull request between existing refs; never writes git objects.", PullCreateInput, false),
    tool("github.pulls.comment.create", "Add one bounded conversation comment to a pull request.", CommentCreateInput, false),
    tool("github.pulls.review.create", "Submit one review without inline comments.", ReviewCreateInput, false),
    ...
    tool("github.commits.check-runs.list", "List bounded check runs for one exact commit ref.", CommitInput),
    tool("github.commits.status.get", "Read the combined commit status for one exact ref.", Schema.Struct({ ...ownerRepo, ref: ShaOrRef })),
];
function asRecord(value, message = "GitHub returned an invalid response.") {
...
function parseCredential(bytes) {
    ...
    return {
        version: 1,
        accessToken,
        accessTokenExpiresAt: expiring ? accessTokenExpiresAt : null,
        refreshToken: expiring ? boundedString(refreshToken, MAX_TOKEN_CHARS) : null,
        refreshTokenExpiresAt: expiring ? refreshTokenExpiresAt : null,
        account: { login, id },
        grantedCapabilities: validateCapabilities(value.grantedCapabilities),
        updatedAt: value.updatedAt,
    };
}
function parseTokenGrant(json) {
...
function listResult(value, limit) {
...
export class GitHubProvider {
    ...
    async #request(url, init, maximumBytes) {
        if (this.#closed)
            throw new Error("GitHub provider is closed.");
    ...
    #writeCredential(value, signal) {
        return Effect.runPromise(this.#secrets.set(GITHUB_SECRET_SUFFIX, encoder.encode(JSON.stringify(value))), { signal });
    }
    ...
    async #api(path, token, options = {}) {
        ...
        const { response, json } = await this.#request(`${GITHUB_API_ORIGIN}${path}`, {
        ...
        if (!response.ok) {
            ...
            if (response.status === 403 && response.headers.get("x-ratelimit-remaining") === "0")
                throw new IntegrationProviderPublicError("GitHub is rate limiting this account. Try again after the rate-limit reset.");
            if (response.status === 403)
                throw new IntegrationProviderPublicError("GitHub denied this request. Check the GitHub App installation, repository selection, and permissions.");
            if (response.status === 404)
                throw new IntegrationProviderPublicError("GitHub could not find that item, or the installed app cannot access it.");
            if (response.status === 409)
                throw new IntegrationProviderPublicError("GitHub could not complete the request because the repository state conflicts with it.");
            if (response.status === 422)
                throw new IntegrationProviderPublicError("GitHub rejected the requested change. Check names, refs, and current repository state.");
            if (response.status === 429)
                throw new IntegrationProviderPublicError("GitHub is temporarily rate limiting requests. Try again later.");
    ...
    async #verify(token, signal) {
        ...
        collectionField(await this.#api("/user/installations?per_page=1&page=1", token, { signal }), "installations", 1);
        return account;
    }
    async status(context) {
        ...
        try {
            ...
            for (const [id, flow] of this.#pending)
                if (flow.expiresAt <= now && !this.#polling.has(id))
            ...
            const accessExpiry = credential.accessTokenExpiresAt === null
                ? null
                : Date.parse(credential.accessTokenExpiresAt);
            const refreshExpiry = credential.refreshTokenExpiresAt === null
                ? null
            ...
            return {
                ...
                accountLabel: credential.account.login,
                grantedCapabilities: credential.grantedCapabilities,
                message: accessExpiry !== null && accessExpiry <= now
    ...
    async connect(capabilities, context, submission) {
        if (submission !== undefined)
            throw new Error("GitHub device sign-in rejects submissions.");
    ...
    async poll(flowId, context) {
        ...
        if (!flow)
            throw new IntegrationProviderPublicError("GitHub sign-in flow was not found. Start sign-in again.");
        ...
        try {
            ...
            admitted = true;
            const { response, json: raw } = await this.#postAuth(TOKEN_URL, {
            ...
            const credential = {
                ...
                accessTokenExpiresAt: grant.expiresAt === null ? null : new Date(grant.expiresAt).toISOString(),
                refreshToken: grant.refreshToken,
                refreshTokenExpiresAt: grant.refreshExpiresAt === null ? null : new Date(grant.refreshExpiresAt).toISOString(),
                account,
    ...
    prepare(context) {
        return this.#serialize(async () => {
            ...
            try {
                ...
                if (!response.ok)
                    throw new IntegrationProviderPublicError("GitHub access could not be refreshed. Reconnect GitHub.");
    ...
    #require(capability) {
        ...
        if (!access ||
            ...
            throw new IntegrationProviderPublicError("GitHub access needs preparation or reconnection before use.");
    ...
    async invoke(toolName, input, context) {
        ...
        if (toolName === "github.installations.list") {
            ...
            const p = page(values);
            return collectionField(await read(`/user/installations?${params({ per_page: p.limit, page: p.page })}`), "installations", p.limit);
        }
        if (toolName === "github.repositories.list") {
            const values = await this.#decode(RepositoriesListInput, input);
            const p = page(values);
            return collectionField(await read(`/user/installations/${values.installationId}/repositories?${params({ per_page: p.limit, page: p.page })}`), "repositories", p.limit);
        }
        if (toolName === "github.repositories.get") {
            const values = await this.#decode(RepositoryGetInput, input);
            return asRecord(await read(`/repos/${encodeURIComponent(values.owner)}/${encodeURIComponent(values.repo)}`));
        }
        if (toolName === "github.repositories.search") {
            const values = await this.#decode(RepositorySearchInput, input);
            const p = page(values);
            return collectionField(await read(`/search/repositories?${params({ q: values.query.trim(), per_page: p.limit, page: p.page })}`), "items", p.limit);
        }
        if (toolName === "github.contents.get") {
            ...
            const result = await read(`/repos/${encodeURIComponent(values.owner)}/${encodeURIComponent(values.repo)}/contents/${values.path.split("/").map(encodeURIComponent).join("/")}${suffix}`, CONTENT_RESPONSE_BYTES);
            if (Array.isArray(result))
                throw new IntegrationProviderPublicError("That path is a directory. Provide an exact file path.");
            ...
            if (file.type !== "file" ||
                ...
                throw new IntegrationProviderPublicError("GitHub file content is unavailable or exceeds the one-megabyte limit.");
            return file;
        }
        if (toolName === "github.code.search") {
            ...
            return collectionField(await read(`/search/code?${params({ q, per_page: p.limit, page: p.page })}`), "items", p.limit);
        }
        if (toolName === "github.issues.list") {
            const values = await this.#decode(IssuesListInput, input);
            const p = page(values);
            const labels = values.labels?.join(",");
            return listResult(await read(`/repos/${encodeURIComponent(values.owner)}/${encodeURIComponent(values.repo)}/issues?${params({ state: values.state ?? "open", labels, per_page: p.limit, page: p.page })}`), p.limit);
        }
        if (toolName === "github.issues.search") {
            const values = await this.#decode(IssuesSearchInput, input);
            const p = page(values);
            const q = `${values.query.trim()} repo:${values.owner}/${values.repo}`;
            return collectionField(await read(`/search/issues?${params({ q, per_page: p.limit, page: p.page })}`), "items", p.limit);
        }
        if (toolName === "github.issues.get") {
            const values = await this.#decode(NumberInput, input);
            return asRecord(await read(`/repos/${encodeURIComponent(values.owner)}/${encodeURIComponent(values.repo)}/issues/${values.number}`));
        }
        if (toolName === "github.issues.comments.list" ||
            toolName === "github.pulls.review-comments.list" ||
            toolName === "github.pulls.reviews.list") {
            const values = await this.#decode(Schema.Struct({ ...ownerRepo, number: IssueNumber, ...pagination }), input);
            const p = page(values);
            const tail = toolName === "github.issues.comments.list"
                ? `issues/${values.number}/comments`
                : toolName === "github.pulls.reviews.list"
                    ? `pulls/${values.number}/reviews`
                    : `pulls/${values.number}/comments`;
            return listResult(await read(`/repos/${encodeURIComponent(values.owner)}/${encodeURIComponent(values.repo)}/${tail}?${params({ per_page: p.limit, page: p.page })}`), p.limit);
        }
        if (toolName === "github.issues.create") {
            const values = await this.#decode(IssueCreateInput, input);
            return write(`/repos/${encodeURIComponent(values.owner)}/${encodeURIComponent(values.repo)}/issues`, "POST", {
                title: values.title,
        ...
        if (toolName === "github.issues.update") {
            ...
            if (Object.keys(body).length === 0)
                throw new IntegrationProviderPublicError("Issue update must include at least one changed field.");
            return write(`/repos/${encodeURIComponent(values.owner)}/${encodeURIComponent(values.repo)}/issues/${values.number}`, "PATCH", body);
        }
        ...
        if (toolName === "github.pulls.list") {
            ...
            const p = page(values);
            return listResult(await read(`/repos/${encodeURIComponent(values.owner)}/${encodeURIComponent(values.repo)}/pulls?${params({ state: values.state ?? "open", base: values.base, head: values.head, per_page: p.limit, page: p.page })}`), p.limit);
        }
        if (toolName === "github.pulls.get") {
            const values = await this.#decode(NumberInput, input);
            return asRecord(await read(`/repos/${encodeURIComponent(values.owner)}/${encodeURIComponent(values.repo)}/pulls/${values.number}`));
        }
        if (toolName === "github.pulls.create") {
            ...
            return write(`/repos/${encodeURIComponent(values.owner)}/${encodeURIComponent(values.repo)}/pulls`, "POST", {
                title: values.title,
                head: values.head,
                base: values.base,
                ...(values.body === undefined ? {} : { body: values.body }),
        ...
        if (toolName === "github.pulls.review.create") {
            ...
            return write(`/repos/${encodeURIComponent(values.owner)}/${encodeURIComponent(values.repo)}/pulls/${values.number}/reviews`, "POST", { event: values.event, body: values.body });
        }
        if (toolName === "github.actions.runs.list") {
            const values = await this.#decode(ActionsRunsListInput, input);
            const p = page(values);
            return collectionField(await read(`/repos/${encodeURIComponent(values.owner)}/${encodeURIComponent(values.repo)}/actions/runs?${params({ branch: values.branch, status: values.status, per_page: p.limit, page: p.page })}`), "workflow_runs", p.limit);
        }
        if (toolName === "github.actions.run.get") {
            const values = await this.#decode(RunInput, input);
            return asRecord(await read(`/repos/${encodeURIComponent(values.owner)}/${encodeURIComponent(values.repo)}/actions/runs/${values.runId}`));
        }
        if (toolName === "github.actions.jobs.list") {
            const values = await this.#decode(JobsInput, input);
            const p = page(values);
            return collectionField(await read(`/repos/${encodeURIComponent(values.owner)}/${encodeURIComponent(values.repo)}/actions/runs/${values.runId}/jobs?${params({ per_page: p.limit, page: p.page })}`), "jobs", p.limit);
        }
        if (toolName === "github.commits.check-runs.list") {
            const values = await this.#decode(CommitInput, input);
            const p = page(values);
            return collectionField(await read(`/repos/${encodeURIComponent(values.owner)}/${encodeURIComponent(values.repo)}/commits/${encodeURIComponent(values.ref)}/check-runs?${params({ per_page: p.limit, page: p.page })}`), "check_runs", p.limit);
        }
        if (toolName === "github.commits.status.get") {
            const values = await this.#decode(Schema.Struct({ ...ownerRepo, ref: ShaOrRef }), input);
            return asRecord(await read(`/repos/${encodeURIComponent(values.owner)}/${encodeURIComponent(values.repo)}/commits/${encodeURIComponent(values.ref)}/status`));
        }
