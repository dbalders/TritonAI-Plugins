...
const refreshValueField = ["refresh", "Token"].join("");
...
const MailFolderListInput = Schema.Struct({
    parentFolderId: Schema.optionalKey(Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(512)).annotate({
...
const MailFolderCreateInput = Schema.Struct({
    displayName: Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(255), Schema.isPattern(/\S/u)).annotate({ description: "Display name for the new mail folder." }),
    parentFolderId: Schema.optionalKey(Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(512)).annotate({
        description: "Optional exact parent mail-folder identifier.",
    })),
});
const MailMessageMoveInput = Schema.Struct({
    ...
    destinationFolderId: Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(512), Schema.isPattern(/\S/u)).annotate({
        description: 'Exact destination mail-folder identifier, or the well-known folder name "archive".',
    }),
});
const CalendarEventsInput = Schema.Struct({
    start: Schema.optionalKey(Schema.String.check(Schema.isMaxLength(64)).annotate({
        description: "Inclusive ISO 8601 start timestamp.",
    })),
    end: Schema.optionalKey(Schema.String.check(Schema.isMaxLength(64)).annotate({
...
const DraftFileAttachment = Schema.Struct({
    ...
    contentBytes: Schema.String.check(Schema.isMaxLength(MAX_DRAFT_ATTACHMENT_BASE64_CHARS), Schema.isPattern(BASE64_PATTERN)).annotate({
...
const MailDraftCreateInput = Schema.Struct({
    ...
    body: Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(MAX_INPUT_BODY_CHARS)).annotate({ description: "Plain-text draft body." }),
...
const CalendarEventFields = {
    ...
    body: Schema.optionalKey(Schema.String.check(Schema.isMaxLength(MAX_INPUT_BODY_CHARS)).annotate({
...
export const MICROSOFT_GRAPH_TOOLS = [
    ...
    },
    {
        ...
        description: "Read one exact Microsoft 365 mail message through a fixed bounded projection; body.truncated reports provider truncation.",
        input: MailGetInput,
        readOnly: true,
        destructive: false,
        idempotent: true,
    ...
    },
    {
        ...
        description: "List bounded Microsoft 365 mail-folder resources from a fixed read-only endpoint.",
    ...
    },
    {
        ...
        description: 'Move one exact Microsoft 365 mail message to an exact folder or the well-known "archive" folder.',
    ...
    },
    {
        ...
        description: "Read one exact attachment from one exact Microsoft 365 mail message through a fixed projection below the host result ceiling.",
    ...
    },
    {
        ...
        description: "Read Microsoft 365 calendar events through a fixed bounded projection; every non-null preview is marked previewIsPartial.",
    ...
    },
    {
        ...
        description: "Read one exact Microsoft 365 calendar event through a fixed bounded projection; body.truncated reports provider truncation.",
    ...
    },
    {
        ...
        description: "Read one exact attachment from one exact Microsoft 365 calendar event through a fixed projection below the host result ceiling.",
    ...
    },
    {
        ...
        description: "Read bounded message history from one exact Microsoft 365 chat; body is null without content, otherwise body.truncated reports provider truncation.",
...
function boundedOptionalString(value, maximumBytes) {
...
function boundedOptionalHttpsUrl(value, maximumBytes, allowedHosts) {
...
function truncatedUtf8String(value, maximumBytes, maximumCodePoints) {
...
function canonicalScopes(value, required) {
...
function legacyCapabilitiesFromScopes(scopes) {
    return Object.entries(CAPABILITY_SCOPES)
        .filter(([capability, scope]) => capability !== "mail.organize" && scopes.includes(scope))
        .map(([capability]) => capability)
        .toSorted();
}
function canonicalCapabilities(value, scopes) {
    if (!Array.isArray(value) ||
        value.length === 0 ||
        value.some((capability) => typeof capability !== "string" || !CAPABILITY_NAMES.has(capability)) ||
...
function parseCredential(bytes) {
    ...
    if ((value.version !== 1 && value.version !== 2) ||
        !exactKeys(value, new Set(isLegacy
        ...
        value.grantedScopes.some((scope) => typeof scope !== "string" || !Object.values(CAPABILITY_SCOPES).includes(scope)) ||
    ...
    const grantedScopes = [...value.grantedScopes].toSorted();
    return {
        version: 2,
        refreshToken,
        grantedScopes,
...
async function readJson(response, maximumBytes) {
    ...
    if (!reader)
        throw new Error("Microsoft returned an empty response.");
    const chunks = [];
    let total = 0;
    try {
        while (true) {
...
function attachmentListResult(value, limit) {
    const collection = collectionResult(value, limit);
    return boundedResult({
...
function optionalGraphDateTime(value) {
    return value === null || value === undefined ? null : graphDateTime(value);
}
function projectedAttachedItemId(value) {
    const id = boundedOptionalString(value, 512);
    return id === "" ? null : id;
}
function projectedAttachedItem(value) {
    const item = asRecord(value);
    switch (graphODataType(item["@odata.type"])) {
        ...
        case "microsoft.graph.calendarSharingMessage":
            return {
                ...
                isRead: item.isRead === null || item.isRead === undefined ? null : requiredBoolean(item.isRead),
...
function attachmentResult(value) {
    ...
    if (metadata.kind === "file") {
        if (typeof value.contentBytes !== "string" ||
...
function mailFolderReceipt(value) {
    return {
        id: boundedString(value.id, 512),
...
function mailMoveReceipt(value) {
    return {
        id: boundedString(value.id, 512),
...
export class MicrosoftGraphProvider {
    ...
    #postForm(path, form, signal) {
        return this.#request(`${this.#loginRoot}/${path}`, {
            method: "POST",
    ...
    #writeCredential(credential, signal) {
        return Effect.runPromise(this.#secrets.set(MICROSOFT_GRAPH_SECRET_SUFFIX, encoder.encode(JSON.stringify(credential))), { signal });
    }
    ...
    async status(context) {
        ...
        try {
            ...
            for (const [flowId, flow] of this.#pending) {
                if (flow.expiresAt <= now && !this.#polling.has(flowId))
    ...
    async connect(capabilities, context, submission) {
        if (submission !== undefined)
            throw new Error("Microsoft device sign-in rejects submissions.");
        ...
        const { response, json } = await this.#postForm("devicecode", { client_id: this.#clientId, scope: [OFFLINE_SCOPE, ...requestedScopes].join(" ") }, context?.signal);
        if (!response.ok) {
            throw new IntegrationProviderPublicError("Microsoft sign-in could not start. Try again later.");
        }
    ...
    async poll(flowId, context) {
        ...
        if (!flow)
            throw new IntegrationProviderPublicError("Microsoft sign-in flow was not found.");
    ...
    #requireCapability(access, capability) {
        if (!access.grantedCapabilities.includes(capability)) {
            throw new IntegrationProviderPublicError(`Microsoft 365 ${capability} access is not granted.`);
        }
    }
    ...
    async #assertMailFolderOutsideDeletionHierarchy(folderId, accessToken, generation, signal) {
        ...
        for (const wellKnownName of DELETION_MOVE_DESTINATIONS) {
            const folder = await this.#graph(`/me/mailFolders/${wellKnownName}?${selectId.toString()}`, accessToken, { signal });
        ...
        for (let depth = 0; depth < MAX_MAIL_FOLDER_ANCESTRY_DEPTH; depth += 1) {
            ...
            if (deletionFolderIds.has(resolvedFolderId))
                throw deletionFolderError();
    ...
    async invoke(toolName, input, context) {
        ...
        if (toolName === "microsoft365.mail.get") {
            ...
            const result = await this.#graph(`/me/messages/${encodeURIComponent(values.messageId)}?${params.toString()}`, access.value, { preferPlainTextBody: true, signal: context?.signal });
            this.#assertInvocationCurrent(generation);
        ...
        if (toolName === "microsoft365.mail.folder.create") {
            ...
            if (values.parentFolderId) {
                await this.#assertMailFolderOutsideDeletionHierarchy(values.parentFolderId, access.value, generation, commitSignal);
            }
        ...
        if (toolName === "microsoft365.mail.message.move") {
            ...
            const commitSignal = await this.#beginInvocationCommit(context);
            this.#assertInvocationCurrent(generation);
            await this.#assertMailFolderOutsideDeletionHierarchy(destinationFolderId, access.value, generation, commitSignal);
            const result = await this.#graph(`/me/messages/${encodeURIComponent(values.messageId)}/move`, access.value, {
                method: "POST",
        ...
        if (toolName === "microsoft365.calendar.event.get") {
            ...
            const result = await this.#graph(`/me/events/${encodeURIComponent(values.eventId)}`, access.value, { signal: context?.signal });
            this.#assertInvocationCurrent(generation);
        ...
        if (toolName === "microsoft365.chat.messages") {
            ...
            const result = await this.#graph(`/chats/${encodeURIComponent(values.chatId)}/messages?${params.toString()}`, access.value, { signal: context?.signal });
            this.#assertInvocationCurrent(generation);
