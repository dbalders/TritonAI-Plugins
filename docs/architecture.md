...
Manifest `apiVersion` and `manifestVersion` version the package format. Package semantic version
tracks plugin behavior and assets. `compatibility.harness` is the explicit supported Harness range.
Breaking provider ABI or trust-boundary changes require a new Harness contract version or a jointly
reviewed source change and compatibility proof; version ranges must not be widened speculatively.

The initial framework mirrors exact Harness head
`a8ea487eaa506a2013f3b32dd3098e37ade1e359` because that head has no exported SDK. Harness owns the
final structural assignment and catalog adapter. Generic registry, lifecycle, secret-store, RPC,
MCP, and installer changes remain in Harness, not in individual plugins.

Every provider PR must expose its exact validated manifest from the compiled module and define a
`compatibility:harness` script that proves its provider export and exact tool set are structurally
assignable to the Harness checkout named by `TRITONAI_HARNESS_ROOT`. It must also include focused
