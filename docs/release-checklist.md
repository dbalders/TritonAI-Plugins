...
6. Run `pnpm readiness:local` against the exact reviewed Harness worktree.
...
1. Confirm the manifest, package version, compatibility range, capabilities, skills, provider ID,
...
5. Run the package's `compatibility:harness` proof for the provider export, exact tool set, and
